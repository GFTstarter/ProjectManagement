...
public class ResourceResource {
	...
	public String list() throws JsonGenerationException, JsonMappingException, IOException {
		...
		List<Resource> allEntries = this.resourceDao.findAll();
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("{id}")
	public Resource read(@PathParam("id") Long id) {

		this.logger.info("getEntry(id)");

		Resource newsEntry = this.resourceDao.find(id);
		if (newsEntry == null) {
	...
	public Resource create(Resource newsEntry) {

		this.logger.info("create(): " + newsEntry);

		return this.resourceDao.save(newsEntry);
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public Resource update(@FormParam("idResource") Long id,
						   @FormParam("resource") String resource) {
		
		Resource resourceObj = resourceDao.find(id);
		
		resourceObj.setIdResource(id);
		resourceObj.setResource(resource);	
		
		this.logger.info("update(): " + resourceObj);
		return this.resourceDao.save(resourceObj);
	}
